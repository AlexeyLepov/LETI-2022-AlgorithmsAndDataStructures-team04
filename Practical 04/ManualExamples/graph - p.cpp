...
class Set
{
    ...
    public:
        ...
        {
            elems.clear();
            for (vector<Elem*>::iterator iter = univers.begin(); iter != univers.end(); iter++)
            {
                delete(*iter);
            }
            univers.clear();
...
Elem* Set::findMinPar()
{
	Elem* A = nullptr;
	int minPar = INT_MAX;
	for (vector<Elem*>::iterator iter = elems.begin(); iter != elems.end(); iter++)
    {
...
{
	Elem *A = nullptr;
	for (vector<Elem*>::iterator iter = vec.begin(); iter != vec.end(); iter++)
    {
...
Set::Set(char nm, vector<string> els):name(nm)
{
	int id;
	Elem *A;
	for (vector<string>::iterator iter = els.begin(); iter != els.end(); iter++)
    {
		id = atoi((*iter).c_str());
		if (id)
        {
...
class Graph
{
    ...
    public:
        ...
        ~Graph()
        {
            for (vector<Set*>::iterator iter = sets.begin(); iter != sets.end(); iter++)
            {
                (*iter)->~Set();
            }
            sets.clear();
